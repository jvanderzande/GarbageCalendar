...
function Perform_Update()
   ...
   -- reformat any change of schedule
   Web_Data = Web_Data:gsub('( uitzondering\\".-uitzondering.tekst)', '')
   --
   for web_garbagetype, web_garbagedate in string.gmatch(Web_Data, '.-soort.(.-)%sclearfix.-text dag\\">(.-)<\\/span') do
      ...
      dprint(i .. ' web_garbagetype:' .. tostring(web_garbagetype) .. '   web_garbagedate:' .. tostring(web_garbagedate) .. '   2web_garbagedate:' .. tostring(url_unescape(web_garbagedate)))
      local dateformat = '????????'
      -- Get days diff
